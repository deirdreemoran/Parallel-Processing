...
*  Program: p2_fg_dyn.cpp
*  Date: 4/29/2018
*  Descripton: Fine grained dynamic OpenMP N-body Problem
				...
				for( int i = 0; i < NUMBODIES; i++ )
                 {
						 ...
						 #pragma omp parallel for schedule(dynamic), reduction(+: fx), reduction(+: fy), reduction(+: fz)// schedule(static)
                         for( int j = 0; j < NUMBODIES; j++ )
                          ...
                          float ax = myfx / Bodies[i].mass;
                          float ay = myfy / Bodies[i].mass;
                          float az = myfz / Bodies[i].mass;
